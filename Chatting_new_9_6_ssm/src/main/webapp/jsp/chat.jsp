...
<script type="text/javascript">
	...
	var toId;
	var ws;
	var target = "ws://localhost:8080/Chatting_new_9_6_ssm/chat?userId=" + userId;
	var tog;	//群id
	$(function() {
								...
								if (tog == result.getmsgid) { //判断用户是否打开了与发送者对应的聊天区(接收方点击的好友ID与消息来源的Id是否相等)																						
									...
									if(result.aj=="1"){
										/* $.get("/Chatting_new_9_1/chat/gmsgread", { "getmsgid" : tog,
										 ...
										 $.ajax({
											type: "POST",
											url : "/Chatting_new_9_6_ssm/chat/gmsgread",
											data : {
												getmsgid : tog,
												myId : userId
											} 
					...
					if(!result.hasOwnProperty("gresp")){ //判断result中是否包含"resp"
						...
						if(result.gresp=="1"){
								...
								if($("#groups").find("#g"+result.getmsgid).length == 0){//	注意 站在请求发送方的角度 添加对方的Id	
									...
									$("#createg").before("<button id='g"+tog+"' type='button' class='btn btn-info btn-block'>"+result.gname+"</button>");
	...
	$("#gsearch").click(function() {
			...
			$("#createGroup").click(function(){
				 $.ajax({
					...
					url:"../group/create",
					data:{
						creategname:$("#creategname").val(),
						uid:userId
					},
					success:function(g){
						if(undefined!=g&&""!=g&&null!=g){
							var gid=g.mygroup.gid;
							var gname=g.mygroup.gname;
							$("#createg").before("<button id='g"+gid+"' type='button' class='btn btn-info btn-block' onclick='groclick("+gid+");'>"+gname+"</button>");
							//$("#groups").on('click','#g'+gid,function(){groclick(gid);}); 
						}
					}
				}); 
			});
						...
						<div id="myTabContent" class="tab-content">
							...
							<div class="tab-pane" id="groups"><!-- 群组 -->
								<c:forEach items="${groups}" var="g" varStatus="vs">
								  ...
								  <div id="createg">
									  <button type="button" class="btn btn-primary btn-block">创建群组</button>
								      <input type="text" class="form-control" id="creategname" placeholder="请输入要创建的群名称" style="width: 80%;display: inline;">
									  <button id="createGroup" class="btn btn-success btn-sm" type="button" style="float: right;width: 20%">提交</button>
								  </div>
